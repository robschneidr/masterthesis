# -*- coding: utf-8 -*-
"""
Created on Wed May  3 16:11:00 2023
...
def compute(nodes, query_factors, false_factor_values, edge_init):
    ...
    computed_trusts = []
    for n in nodes:
        if random.random() < n.willingness_to_compute:
            computed_trusts.append(set_trust(n , query_factors, false_factor_values, edge_init))
...
def set_trust(node, query_factors, false_factor_values, edge_init):
    new_trust = edge_init  
...
def HITS_iteration(nodes, n_search_queries, n_steps=5,
    ...
    for nth_query in range(n_search_queries):
        ...
        avg_computed_trust = compute(nodes, query_factors, false_factor_values, edge_init)
