...
def create_random_weighted_directed_document_nodes(n_nodes, n_edges):
    ...
    #normalize
    H.normalize_trusts(nodes, sum(n.trust for n in nodes))
    H.normalize_auths(nodes, sum(n.auth for n in nodes))
    H.normalize_hubs(nodes, sum(n.hub for n in nodes))
